  ...
  public Result<Map<String, Object>> contextChat(@RequestBody ContextChatRequest request) {
    try {
      ...
      if (contextMessages.size() <= 1) {
        // 首次对话或只有当前消息，直接调用
        response = chatClient.prompt().system(systemPrompt).user(request.getMessage()).call().content();
      } else {
        // 有历史消息，构建完整的上下文提示词
        ...
        response = chatClient.prompt().system(contextPrompt.toString()).user(request.getMessage()).call()
                .content();
      }

      // 添加AI回复到会话历史
      String messageType = request.getUseKnowledge() != null && request.getUseKnowledge() ? "knowledge_based"
              : "text";
  ...
  public SseEmitter knowledgeBasedStreamChat(@RequestBody KnowledgeChatRequest request) {
    ...
    try {
      ...
      final String sessionId = request.getSessionId() != null
              && chatSessionService.getSession(request.getSessionId()) != null ? request.getSessionId()
      ...
      if (request.getUseContext() != null && request.getUseContext()) {
        ...
        if (contextMessages.size() <= 1) {
          // 首次对话，直接调用
          flux = chatClient.prompt().system(systemPrompt).user(request.getMessage()).stream().content();
        } else {
          // 有历史消息，构建完整的上下文提示词
          StringBuilder contextPrompt = new StringBuilder(systemPrompt);
          contextPrompt.append("\n\n以下是对话历史：\n");
          ...
          flux = chatClient.prompt().system(contextPrompt.toString()).user(request.getMessage()).stream()
                  .content();
        }
      } else {
        // 单轮对话模式
      ...
      flux.subscribe(contentChunk -> {
        try {
          synchronized (responseBuilder) {
            responseBuilder.append(contentChunk);
          }
          emitter.send(SseEmitter.event().name("message").data(contentChunk));
        } catch (Exception e) {
        ...
        try {
          // 添加完整的AI回复到会话历史
          chatSessionService.addMessage(sessionId, "assistant", responseBuilder.toString(),
                  "knowledge_stream");
      ...
      try {
        emitter.send(SseEmitter.event().name("error").data("知识库服务暂时不可用"));
      } catch (Exception ignored) {
      }
      emitter.completeWithError(e);
    }
